...
class Server {
	public static void main(String[] argv) {
		try {
			...
			// Ensure we can detect the server not being stopped properly.
			var notACleanExitFile = new File("NotACleanExit");
			notACleanExitFile.createNewFile();
			pause();
			// This signals the server is ready to accept connections.
			print("Done");
			while (true) {
	...
	// These gamerules are integers,
	// the rest is assumed to be boolean with 'false' as default.
	static {
	...
	static void handleGameruleCommand(Scanner scanner) {
		if (scanner.hasNext()) {
			...
			if (scanner.hasNext()) {
				...
				var value = gamerules.containsKey(gamerule)
						? gamerules.get(gamerule)
						: false;
				print("Gamerule " + gamerule
						+ " is currently set to: "
						+ value);
